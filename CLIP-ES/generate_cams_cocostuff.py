...
from clip_text import class_names_coco_stuff182_dict, coco_stuff_182_to_27, coco_stuff_182_to_171, coco_stuff_171_to_27, coco_stuff_27_to_182
#new_class_names_coco, BACKGROUND_CATEGORY_COCO
import argparse
...
def perform(process_id, dataset_list, args, model, bg_text_features, fg_text_features, cam, split_label_list):
    ...
    for im_idx, im in enumerate(tqdm(databin)):
        ...
        for lid in label_id_list:
            label_list.append(new_class_names_cocostuff171[int(lid)])
        ...
        for image in ms_imgs:
            ...
            for idx, label in enumerate(label_list):
                keys.append(new_class_names_cocostuff171.index(label))
...
if __name__ == "__main__":
    ...
    '''
    new_class_names_cocostuff27 = []
    for coarse, fine_list in coco_stuff_27_to_182.items():
        name_str = ''
        for idx, fine in enumerate(fine_list):
            if fine + 1 in [12, 26, 29, 30, 45, 66, 68, 69, 71, 83, 91]:
                continue
            name_str += class_names_coco_stuff182_dict[fine+1]
            if idx < len(fine_list)-1:
                name_str += ','
        new_class_names_cocostuff27.append(name_str)
        print(name_str)
    '''
    new_class_names_cocostuff171 = []
    for k in class_names_coco_stuff182_dict:
        if k in [0, 12, 26, 29, 30, 45, 66, 68, 69, 71, 83, 91]:
            continue
        new_class_names_cocostuff171.append(class_names_coco_stuff182_dict[k])
    print(len(new_class_names_cocostuff171))

    model, _ = clip.load(args.model, device=device)
    #bg_text_features = None
    bg_text_features = zeroshot_classifier(['background'], ['a clean origami {}.'], model)#['a rendering of a weird {}.'], model)
    fg_text_features = zeroshot_classifier(new_class_names_cocostuff171, ['a clean origami {}.'], model)#['a rendering of a weird {}.'], model)
