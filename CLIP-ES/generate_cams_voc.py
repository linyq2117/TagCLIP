...
def perform(process_id, dataset_list, args, model, bg_text_features, fg_text_features, cam):
    ...
    for im_idx, im in enumerate(tqdm(databin)):#(tqdm(databin)):
        '''
        ...
        for image in ms_imgs:
            ...
            for idx, label in enumerate(label_list):
                ...
                if idx == 0:
                    ...
                    attn_weight = torch.stack(attn_weight, dim=0)[-8:-1]#[-8:]
...
if __name__ == "__main__":
    ...
    train_list = [x[0] + '.jpg' for x in file_list]  # [8000:]
    all_label_list = [x[1:] for x in file_list]  # [8000:]
    print('>>>>generating {}'.format(args.split_file))
    print('image cnt:', len(train_list))
