...
from clip.multi_head_attn import MultiheadAttention2
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, need_weights: bool = True):#False):
        super().__init__()

        #self.attn = nn.MultiheadAttention(d_model, n_head)
        self.attn = MultiheadAttention2(d_model, n_head)
    ...
    def attention(self, x: torch.Tensor, mask=None, layer=None):
        if x.shape[0] != 77 and mask is not None:
            if isinstance(mask,int):
                if layer == 11:
                    self.attn_mask =1 - torch.eye(x.shape[0], x.shape[0])
                    self.attn_mask = self.attn_mask * float(-100)#float('-inf')#float(-100)
                else:
                    self.attn_mask = None
            else:
                self.attn_mask = mask
    ...
    def forward(self, x: torch.Tensor, mask=None, layer=None):
        if self.need_weights == False:
            x = x + self.attention(self.ln_1(x))
            x = x + self.mlp(self.ln_2(x))
            return x
        else:
            attn_output, attn_weight, q, k, v = self.attention(self.ln_1(x), mask=mask, layer=layer)#(L,N,E)  (N,L,L)
            x = x + attn_output
            x = x + self.mlp(self.ln_2(x))
            return x, attn_weight, q, k, v
            


class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, need_weights: bool = True):#False):
        super().__init__()
        self.width = width
        self.layers = layers
    ...
    def forward(self, x: torch.Tensor, attn_mask=None):
        ...
        with torch.no_grad():
            for i in range(self.layers):
                x, attn_weight, q, k, v = self.resblocks[i](x, mask=attn_mask, layer=i)
                attn_weights.append(attn_weight)
                feature_map_list.append(x)
                v_list.append(k)
        return x, attn_weights, feature_map_list, v_list
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, H, W, attn_mask=None):
        if H == 224 and W == 224:
        ...
        x, attn_weight, feature_map_list, v_list = self.transformer(x, attn_mask=attn_mask)
        ...
        v_list = [v.permute(1,0,2)[:,1:,:] for v in v_list]

        return x, feature_map_list, attn_weight, v_list
        #return x
...
class CLIP(nn.Module):
    ...
    def encode_image_tagclip(self, image, H, W,attn_mask=None, use_clstoken=False):
        x,feamap,attn,v =  self.visual(image.type(self.dtype), H, W, attn_mask=attn_mask)
        if use_clstoken:
            x = x[:,0,:]
        return x, attn
    ...
    def encode_image(self, image):
        return self.visual(image.type(self.dtype), 224, 224)
